...
(function() {
     ...
     * @param {number} The key of icon format to load, where 0 specifies for icon font and 1 for SVG image
    ...
    function appendIcon(element, icon_key, format_key=0) {
        ...
        element.innerHTML += ` <p class="qclt-icon">${icon}</p>`;
        return icon;
    }
    ...
    async function main () {
        ...
        if (pathRe[1] == "/focuses") {
            ...
            let el = document.querySelector("div.header__title > h1");
            const id = pathRe[2];
            ...
            if (pathRe[1] == "/quests") {
                ...
                let el = document.querySelector(".ql-headline-1");
                const id = pathRe[2];
                const title = el.innerText;
                switch (await getQuestStatusById(id)) {
                    case "finished":
